...
public final class HollowerConfigScreen extends Screen {
    ...
    private void buildRouteTab() {
        ...
        actions.add(tip(action("Optimize Order...", nodes > 3,
                        button -> ClientCompat.setScreen(
                                Minecraft.getInstance(), new RouteOptimizeScreen(this))),
                "Reorder this route's nodes into a shorter, straighter path.\n"
                        + "The nodes stay put; only the order you visit them in changes."
                        + (nodes > 3 ? "" : "\nUnavailable: needs at least four nodes.")));
